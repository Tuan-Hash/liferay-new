...
export const MultipleReference = React.memo((props) => {
  ...
  const sendDataSubmit = (arr) => {
    let dataSubmit = [];

    referenceList.forEach(row => {
      arr.forEach(select => {
        if (row[options.label] == select) {
          dataSubmit.push(row[options.key])
        }
      });
    });
            ...
            startAdornment: (valueList.length == 0 ? null :
              <InputAdornment position="start">
                {valueList.map(select => {
                  return (
                    <Chip style={{marginRight: 5, height: 25 }} label={select} variant="outlined" onDelete={() => { handleDelete(select) }} />
                  )
                })}
              </InputAdornment>
            ),
